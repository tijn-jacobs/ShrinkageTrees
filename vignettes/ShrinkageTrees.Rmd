...
the penalised problem separately for every draw, at a common $\lambda$. The
lasso is a nonlinear operator, so the projection of the posterior mean is not
the mean of the projections. A per-draw solution gives a genuine posterior
over sparse summaries: a covariate enters some draws and not others. The
`Nonzero` column reports the fraction of draws in which each covariate
enters.

The penalty $\lambda$ indexes the summary family, so we fix it once for all
draws. We choose it by `cv.glmnet` on the posterior mean, at `lambda.1se`.
