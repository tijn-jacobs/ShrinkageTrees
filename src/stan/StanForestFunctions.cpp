...
double getpb(StanTree& t, xinfo& xi, pinfo& pi, StanTree::npv& goodbots)
{
   ...
   StanTree::npv leaves; //all the bottom nodes
   t.getbots(leaves);
   for(size_t i=0;i!=leaves.size();i++)
      if(cansplit(leaves[i],xi)) goodbots.push_back(leaves[i]);
   if(goodbots.size()==0) { //are there any bottom nodes you can split on?
      pb=0.0;
   } else {
      if(t.StanTreesize()==1) pb=1.0; //is there just one node?
      else pb=pi.pb;
   }
   return pb;
}
...
void getsuff(StanTree& x, StanTree::StanTree_p l, StanTree::StanTree_p r, xinfo& xi, dinfo& di, size_t& nl, double& syl, size_t& nr, double& syr)
{
   ...
   for(size_t i=0;i<di.n;i++) {
      xx = di.x + i*di.p;
      StanTree::StanTree_cp bn = x.bn(xx,xi);
      if(bn==l) {
         nl++;
         syl += di.y[i];
      }
...
{
   StanTree::StanTree_cp tbn; //the pointer to the bottom node for the current observations
   ...
   typedef StanTree::npv::size_type bvsz;
   bvsz nb = leaves.size();
   nv.resize(nb);
   syv.resize(nb);

   std::map<StanTree::StanTree_cp,size_t> bnmap;
   for(bvsz i=0;i!=leaves.size();i++) {bnmap[leaves[i]]=i;nv[i]=0;syv[i]=0.0;}
...
*/
      ...
      StanTree::StanTree_p nxp = nx->getp();
