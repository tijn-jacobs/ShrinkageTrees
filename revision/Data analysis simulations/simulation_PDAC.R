...
data_gen <- function(n_train, p_feat, sigma, s_prog, s_treat, cens_scale, linear = NULL) {
  ...
  # Prognostic function f(x)
  beta_f <- rnorm(p, 0, 2) * rbinom(p, 1, s_prog)
  f_x <- 6 + as.numeric(beta_f %*% t(X_train))
  
  # Treatment effect tau(x)
  beta_tau <- rnorm(p, 1, 0.5) * rbinom(p, 1, s_treat)
  tau_x <- 2 + as.numeric(beta_tau %*% t(X_train))
  
  true_ate_raw <- sum(beta_tau * colMeans(X_train))
  ...
  # Standardise scale (as in your simulations)
  sd_un <- sd(log_T)
...
# 
# 
# medium <- readRDS("/Users/tijnjacobs/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/simulations revision/Data analysis simulations/simulation_PDAC_output.rds")
# 
# results <- medium$results_medium
# 
# # Sanity check
