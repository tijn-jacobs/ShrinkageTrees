...
p_vals <- round(10^seq(log10(10), log10(2000), length.out = 25)) # Important to set number of p's!!!
...
cens_scales_medium <- c(
  0.04959749, 0.04996240, 0.05003666, 0.04930298, 0.04940034,
  0.04963236, 0.05144284, 0.04952161, 0.04902881, 0.05017515,
  0.04881927, 0.04894963, 0.05074934, 0.05055998, 0.04691268,
  0.04665767, 0.04676176, 0.04294362, 0.04355119, 0.04228465,
  0.04355119, 0.04122862, 0.03990283, 0.03826993, 0.04485325
)
...
output_file <- file.path(Sys.getenv("TMPDIR"), "quick2_output.rds")
...
medium <- readRDS("/Users/tijnjacobs/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/simulations revision/quick2_output.rds")
...
smooth_fun <- geom_smooth(
  se = FALSE,
  method = "loess",
  span = 0.4,      # Adjust this to tune smoothness
  linewidth = 0.9
)
