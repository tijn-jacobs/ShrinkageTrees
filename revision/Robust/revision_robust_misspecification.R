...
data_gen <- function(n_train, p_feat, sigma, s_prog, s_treat, cens_scale = 1, 
                     linear, error_dist) {
  ...
  if (error_dist == "logistic") {
    error <- rlogis(n_train, location = 0, scale = 3/pi)
  } else if (error_dist == "gumbel") {
    error <- rgumbel(n_train, loc = -sqrt(18)/pi * 0.5772157, scale = sqrt(18)/pi)
  } else {
    error <- rnorm(n_train, 0, sigma)
  }
  

  true_event_times <- mu + (treatment - 0.5) * tau
  uncensored_event_times <- true_event_times + error
...
run_single_simulation <- function(n_train,
  ...
  while (attempt <= max_attempts && !success) {
    message(sprintf("Attempt %d of %d", attempt, max_attempts))
    
    # Generate data
    dt <- data_gen(n_train, p_feat, sigma, s_prog, s_treat, cens_scale, linear, error_dist)
    
    try_res <- try({
      # Evaluate IndivAFT SP
      results_indiv_aft_SP <- evaluate_IndivAFT(
        dt,
        nskip = N_burn,
        ndpost = N_post,
        ntree = 200,
      ...
      # Evaluate IndivAFT NP
      results_indiv_aft_NP <- evaluate_IndivAFT(
        dt,
        nskip = N_burn,
        ndpost = N_post,
        ntree = 200,
      ...
      # If all succeed, set success to TRUE
      success <- TRUE
    }, silent = TRUE)
    
    if (!success) {
      message("Error detected — regenerating new data and retrying.")
      attempt <- attempt + 1
    }
  }
  
  if (!success) {
    stop("Maximum number of attempts reached. Could not complete simulation successfully.")
  }
  
  # Once successful, return results
  return(data.frame(
    Method = c("IndivAFT (SP)", "IndivAFT (NP)", "CHF (k=0.1)"),
    sigma = c(results_indiv_aft_SP$postmean_sigma,
              results_indiv_aft_NP$postmean_sigma,
              results_chf0.1$postmean_sigma),
    ATE = c(results_indiv_aft_SP$ate,
            results_indiv_aft_NP$ate,
            results_chf0.1$ate),
    ATE_bias = c(results_indiv_aft_SP$ate - dt$true_ate,
                 results_indiv_aft_NP$ate - dt$true_ate,
                 results_chf0.1$ate - dt$true_ate),
    ATE_coverage = c(results_indiv_aft_SP$ate_coverage,
                     results_indiv_aft_NP$ate_coverage,
                     results_chf0.1$ate_coverage),
    ATE_CI_Length = c(results_indiv_aft_SP$ate_ci_length,
                      results_indiv_aft_NP$ate_ci_length,
                      results_chf0.1$ate_ci_length),
    CATE_RPEHE = c(results_indiv_aft_SP$cate_rpehe,
                   results_indiv_aft_NP$cate_rpehe,
                   results_chf0.1$cate_rpehe),
    CATE_coverage = c(results_indiv_aft_SP$cate_coverage,
                      results_indiv_aft_NP$cate_coverage,
                      results_chf0.1$cate_coverage),
    CATE_CI_Length = c(results_indiv_aft_SP$cate_ci_length,
                       results_indiv_aft_NP$cate_ci_length,
                       results_chf0.1$cate_ci_length),
    RMSE = c(results_indiv_aft_SP$rmse,
             results_indiv_aft_NP$rmse,
             results_chf0.1$rmse),
    C_Index = c(results_indiv_aft_SP$c_index,
                results_indiv_aft_NP$c_index,
...
find_cens_scale <- function(p,
  ...
  estimate_cens_diff <- function(cens_scale) {
    rates <- replicate(M, {
      dt <- data_gen(n_train, p, sigma, s_prog, s_treat, cens_scale, linear, error_dist)
      mean(1 - dt$status)
    })
...
cat("SIMULATION: robust misspecification\n")
...
cat("\nStarting with linear gumbel scenario\n\n")
# cens_scales <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, linear = TRUE, error_dist = "gumbel"))
cens_scales <- c(0.03972711, 0.03252260, 0.03010766)
results_linear_gumbel <- run_simulations_over_p(
  p_values = p_vals,
...
cat("\nStarting with nonlinear gumbel scenario\n\n")
# cens_scales <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, linear = FALSE, error_dist = "gumbel"))
cens_scales <- c(0.06459536, 0.05177333, 0.03580214)
results_nonlinear_gumbel <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales,
  M = M,
  n_train = n_train,
...
cat("\nStarting with linear logistic scenario\n\n")
# cens_scales <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, linear = TRUE, error_dist = "logistic"))
cens_scales <- c(0.03176408, 0.02982387, 0.02890313)
results_linear_logistic <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales,
  M = M,
  n_train = n_train,
...
cat("\nStarting with nonlinear logistic scenario\n\n")
# cens_scales <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, linear = FALSE, error_dist = "logistic"))
cens_scales <- c(0.06108992, 0.05100871, 0.03516441)
results_nonlinear_logistic <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales,
  M = M,
  n_train = n_train,
...
combined_results <- list(results_linear_gumbel = results_linear_gumbel,
                         results_nonlinear_gumbel = results_nonlinear_gumbel,
                         results_linear_logistic = results_linear_logistic,
                         results_nonlinear_logistic = results_nonlinear_logistic)


# Define output file path 
# (! NAME MUST BE FILENAME_output.rds !)
output_file <- file.path(Sys.getenv("TMPDIR"), "revision_robust_misspecification_output.rds")
