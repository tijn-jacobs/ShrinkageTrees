...
data_gen <- function(n_train, p_feat, sigma, s_prog, s_treat, cens_scale = 1, 
                     effect, error_dist = "normal") {


  X_train <- matrix(runif(n_train*p_feat), nrow = n_train)
  beta <- as.vector(1/(1:p_feat)^2)
  mu <- as.vector(beta %*% t(X_train))
  propensity <- pnorm(mu)
  ...
  if (effect == "linear") {
    ...
    tau <- 1 + X_train[, 1] - (1/2) * X_train[, 2] + (1/3) * X_train[, 3] - 
      (1/4) * X_train[, 4] + (1/5) * X_train[, 5] 
    
    true_ate <- 1 - (1/2) * 0.5 + (1/3) * 0.5 - (1/4) * 0.5 + (1/5) * 0.5 
    
  } else if (effect == "nonlinear") {
    tau <- 10 * sin(pi * X_train[, 1] * X_train[, 2]) +
      20 * (X_train[, 3] - 0.5)^2 +
      10 * X_train[, 4] +
      5 * X_train[, 5]
    
    true_ate <- 10 * 0.5246745 + 20 * 1 / 12 + 10 * 0.5 + 5 * 0.5
    
  } else if (effect == "homogenuous") {
  ...
  if (error_dist == "logistic") {
    error <- stats::rlogis(n_train, location = 0, scale = 3/pi)
  } else if (error_dist == "gumbel") {
    error <- extraDistr::rgumbel(n_train, loc = -sqrt(18)/pi * 0.5772157, scale = sqrt(18)/pi)
  } else {
...
run_single_simulation <- function(n_train,
  ...
  # Generate data
  dt <- data_gen(n_train, p_feat, sigma, s_prog, s_treat, cens_scale, effect, error_dist)
  ...
  return(data.frame(
    Method = c("IndivAFT (SP)", "CHF (k=0.1)"),
...
find_cens_scale <- function(p,
  ...
  estimate_cens_diff <- function(cens_scale) {
    rates <- replicate(M, {
      dt <- data_gen(n_train, p, sigma, s_prog, s_treat, cens_scale, effect, error_dist)
      mean(1 - dt$status)
    })
...
p_vals <- c(100, 300, 1000)
# cens_scales_homo1 <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, sigma = sqrt(1), s_prog = 0.1, s_treat = 0.05, M = 500, effect = "homogenuous", error_dist = "normal"))
cens_scales_homo1 <- c(0.02655513, 0.02562257, 0.02515099)

# cens_scales_homo5 <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, sigma = sqrt(5), s_prog = 0.1, s_treat = 0.05, M = 500, effect = "homogenuous", error_dist = "normal"))
cens_scales_homo5 <- c(0.02729808, 0.02729808, 0.02696460)

# cens_scales_lin1 <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, sigma = sqrt(1), s_prog = 0.1, s_treat = 0.05, M = 500, effect = "linear", error_dist = "normal"))
cens_scales_lin1 <- c(0.02640346, 0.02656309, 0.02544013)

# cens_scales_lin5 <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, sigma = sqrt(5), s_prog = 0.1, s_treat = 0.05, M = 500, effect = "linear", error_dist = "normal"))
cens_scales_lin5 <- c(0.02729808, 0.02729808, 0.02663141)


cat("\nStarting with homogenuous (1) scenario\n\n")
results_homo1 <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_homo1,
  M = M,
  n_train = n_train,
  sigma = sqrt(1),
  s_prog = s_prog,
...
cat("\nStarting with homogenuous (5) scenario\n\n")
results_homo5 <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_homo5,
  M = M,
  n_train = n_train,
  sigma = sqrt(5),
  s_prog = s_prog,
...
cat("\nStarting with linear (5) scenario\n\n")
results_lin1 <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_lin1,
  M = M,
  n_train = n_train,
  sigma = sqrt(1),
  s_prog = s_prog,
...
cat("\nStarting with linear (5) scenario\n\n")
results_lin5 <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_lin5,
  M = M,
  n_train = n_train,
  sigma = sqrt(5),
  s_prog = s_prog,
...
combined_results <- list(results_homo1 = results_homo1,
                         results_homo5 = results_homo5,
                         results_lin1 = results_lin1,
                         results_lin5 = results_lin5)


# Define output file path 
# (! NAME MUST BE FILENAME_output.rds !)
output_file <- file.path(Sys.getenv("TMPDIR"), "robust_dense_output.rds")
