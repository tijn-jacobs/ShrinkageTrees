...
data_gen <- function(n_train, p_feat, sigma, s_prog, s_treat, cens_scale = 1, 
  ...
  block_size <- 50
  num_blocks <- p_feat / block_size
  rho <- 0.8
  
  if (p_feat %% block_size != 0) {
    stop("p_feat must be divisible by block_size.")
  }
  
  make_decay_block <- function(size, rho) {
    mat <- matrix(0, nrow = size, ncol = size)
    for (i in 1:size) {
      for (j in 1:size) {
        mat[i, j] <- rho^abs((i - j) / 2)
      }
    }
    diag(mat) <- 1
    return(mat)
  ...
  blocks_list <- vector("list", num_blocks)
  for (b in seq_len(num_blocks)) {
    Sigma_block <- make_decay_block(block_size, rho)
    Z_block <- MASS::mvrnorm(n = n_train, mu = rep(0, block_size), Sigma = Sigma_block)
    U_block <- pnorm(Z_block)
    blocks_list[[b]] <- U_block
  }
  
  # Combine all blocks
  X_train <- do.call(cbind, blocks_list)
  ...
  if (effect == "linear") {
    ...
    true_ate <- 1 - 2 * 0.5 + 3 * 0.5 - 4 * 0.5 + 5 * 0.5 + sum(beta_prog) * 0.5
...
cat("SIMULATION: robust correlated \n")
...
p_vals <- c(100, 1000, 2000)

cat("\nStarting with linear scenario\n\n")
# cens_scales_linear <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, effect = "linear", error_dist = "normal"))
cens_scales_linear <- c(0.02729808, 0.02799899, 0.03002657)
...
# cens_scales_nonlinear <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, effect = "nonlinear", error_dist = "normal"))
cens_scales_nonlinear <- c(0.06069004, 0.05159488, 0.04478671)
...
cat("\nStarting with homogenuous scenario\n\n")
# cens_scales_homo <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, effect = "homogenuous", error_dist = "normal"))
cens_scales_homo <- c(0.03669489, 0.03169764, 0.02904133)
results_homo <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_homo,
  M = M,
  n_train = n_train,
  sigma = sigma,
  s_prog = s_prog,
...
cat("\nStarting with null scenario\n\n")
# cens_scales_null <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = 0.1, s_treat = 0.05, M = 500, effect = "null", error_dist = "normal"))
cens_scales_null <- c(0.02205648, 0.02729808, 0.02763141)
results_null <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_null,
  M = M,
  n_train = n_train,
  sigma = sigma,
  s_prog = s_prog,
...
output_file <- file.path(Sys.getenv("TMPDIR"), "revision_robust_correlated_output.rds")
