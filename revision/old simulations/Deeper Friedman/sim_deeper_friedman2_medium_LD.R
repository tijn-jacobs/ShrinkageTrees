...
cat("SIMULATION: DEEPER FRIEDMAN2 medium LOW DIM\n")
...
# cens_scales_medium <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, M = 500))
cens_scales_medium <- c(0.06693914, 0.06617644, 0.0666749, 0.06645947, 0.06559974, 0.06635886, 0.06578321, 0.06531664, 0.06466232, 0.06638333, 0.06437722, 0.06323873, 0.0635695, 0.06155998, 0.07079277, 0.05965401, 0.05796302, 0.05642542, 0.05629626, 0.05146102)

# Do the HD scenario now
cens_scales_medium <- cens_scales_medium[p_vals <= 100]
...
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_deeper_friedman2_medium_LD_output.rds")
