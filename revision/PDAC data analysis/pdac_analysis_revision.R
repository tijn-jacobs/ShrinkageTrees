...
# Get summary measures for heterogeneity
# Posterior samples of individual treatment effects τ(x_i)
theta <- fit$train_predictions_sample_treat  # [S × n]

# Posterior average treatment effect per draw
theta_bar <- rowMeans(theta)

# Posterior probability of differential treatment effect (Henderson et al.)
D <- colMeans(sweep(theta, 1, theta_bar, FUN = ">="))
D_star <- pmax(1 - 2 * D, 2 * D - 1)

# Proportion of patients with strong / mild evidence of heterogeneity
prop_strong <- mean(D_star > 0.95)
prop_mild   <- mean(D_star > 0.80)

# Posterior probability of benefit Pr{τ(x_i) > 0}
Pr_pos <- colMeans(theta > 0)

# Summary table (percentages of patients)
summary_table <- data.frame(
  Metric = c(
    "D*_i > 0.95",
    "D*_i > 0.80",
    "Pr(τ(x_i) > 0 | D) ∈ (0.95, 1]",
    "Pr(τ(x_i) > 0 | D) ∈ (0.75, 0.95]",
    "Pr(τ(x_i) > 0 | D) ∈ (0.25, 0.75]",
    "Pr(τ(x_i) > 0 | D) ∈ [0, 0.25]"
  ),
  Percentage = 100 * c(
    mean(D_star > 0.95),
    mean(D_star > 0.80),
    mean(Pr_pos > 0.95),
    mean(Pr_pos > 0.75 & Pr_pos <= 0.95),
    mean(Pr_pos > 0.25 & Pr_pos <= 0.75),
    mean(Pr_pos <= 0.25)
  )
)
...
hist(D_star,
     ...
     border = "white",
     xlab = expression(D[i]^"*"),
     main = "Posterior evidence of differential treatment effect")



# Ensure output directory exists
out_dir <- "~/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/revision/PDAC data analysis"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# Use Times New Roman and larger base size
pdf(
  file = file.path(out_dir, "pdac_Dstar_histogram.pdf"),
  width = 6, height = 6,
  family = "Times"
)
