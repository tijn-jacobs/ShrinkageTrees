...
# ------------------------------------------------------------
fit_bart_ite <- function(X, Z, Y, ndpost, nskip, seed) {
...
metric_ite_coverage <- function(tau_draws, tau_true, alpha) {
...
metric_ite_intlen <- function(tau_draws, alpha) {
  lower <- apply(tau_draws, 2, quantile, probs = alpha / 2)
  upper <- apply(tau_draws, 2, quantile, probs = 1 - alpha / 2)
  mean(upper - lower)
}
...
run_one_rep <- function(setting, rep, X, cfg) {
  ...
  if (cfg$compute_intervals) {
    ite_cov  <- metric_ite_coverage(fit$tau_draws, tau_true, cfg$alpha)
    ite_len <- metric_ite_intlen(fit$tau_draws, cfg$alpha)
  } else {
    ite_cov  <- NA_real_
    ite_len <- NA_real_
  }
...
# Save results
# ------------------------------------------------------------
output_file <- file.path(Sys.getenv("TMPDIR"), "acic5000_output.rds")
...
# Load results
saved <- readRDS(
  "/Users/tijnjacobs/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/simulations revision/acic5000_output.rds"
)
