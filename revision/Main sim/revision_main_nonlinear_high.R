...
cat("SIMULATION: revision main nonlinear\n")
...
param_grid_for_CV <- expand.grid(
  k = c(0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.75, 1)
)

# p_vals <- c(100, 1000, 10000)
# # cens_scales_nonlinear <- sapply(p_vals, function(p) find_cens_scale(p = p, n_train = n_train, sigma = sigma, s_prog = s_prog, s_treat = s_treat, target_cens = 0.35, linear = FALSE, M = 1000))
# cens_scales_nonlinear <- c(0.05612566, 0.04476263, 0.02639552)

p_vals <- c(10000)
cens_scales_nonlinear <- c(0.02639552)
...
info_object <- list(
  ...
  notes = "Nonlinear Friedman-style DGP with high-dimensional covariates; censoring calibrated per p."
)


combined_results <- list(info_object = info_object,
                         results_nonlinear = results_nonlinear)



# Define output file path 
# (! NAME MUST BE FILENAME_output.rds !)
output_file <- file.path(Sys.getenv("TMPDIR"), "revision_main_nonlinear_high_output.rds")
...
# # Load results
# res <- readRDS(
#   "~/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/revision/Main sim/revision_main_nonlinear_high_output.rds"
# )
# 
# # Extract and summarize CATE metrics
# cate_summary <- res$results_nonlinear %>%
#   select(
#     Method, p_feat,
#     CATE_RPEHE,
#     CATE_AbsBias,
