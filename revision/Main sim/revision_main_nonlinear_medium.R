...
p_vals <- c(5000)
# cens_scales_nonlinear <- sapply(p_vals, function(p) find_cens_scale(p = p, n_train = n_train, sigma = sigma, s_prog = s_prog, s_treat = s_treat, target_cens = 0.35, linear = FALSE, M = 500))
cens_scales_nonlinear <- c(0.03028495)
...
output_file <- file.path(Sys.getenv("TMPDIR"), "revision_main_nonlinear_medium_output.rds")
...
# # Load results
# res <- readRDS(
#   "~/Library/CloudStorage/OneDrive-VrijeUniversiteitAmsterdam/Documents/GitHub/ShrinkageTrees/revision/Main sim/revision_main_nonlinear_medium_output.rds"
# )
# 
