...
# cens_scales_linear <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.35, s_prog = s_prog, s_treat = s_treat, M = 500, linear = TRUE))
cens_scales_linear <- c(0.03037056, 0.03000039, 0.02933508)
...
combined_results <- list(results_linear = results_linear)


# Define output file path 
# (! NAME MUST BE FILENAME_output.rds !)
output_file <- file.path(Sys.getenv("TMPDIR"), "main_linear_output.rds")
