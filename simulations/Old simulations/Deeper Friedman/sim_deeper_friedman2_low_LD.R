...
evaluate_IndivAFT <- function(data, use_uncensored = FALSE, ...) {
  ...
  fit <- IndivAFT_new(
    ...
    status = status,
    Trt = data$treatment,
    x.test = matrix(colMeans(data$X_train), nrow = 1),
    sigdf = 3,
    sigquant = 0.5,
    k = 2.0,
    power = 2.0,
...
cat("SIMULATION: DEEPER FRIEDMAN2 low\n")
...
p_vals <- round(10^seq(log10(10), log10(1000), length.out = 20)) # Important to set number of p's!!!
# cens_scales_low <- sapply(p_vals, function(p) find_cens_scale(p, target_cens = 0.10, M = 500))
cens_scales_low <- c(0.01305115, 0.01313885, 0.01310295, 0.01326541, 0.01334416, 0.01314232, 0.01291, 0.0130909, 0.01330776, 0.01309187, 0.01269518, 0.01244127, 0.01265942, 0.01229012, 0.01236764, 0.01216685, 0.0115979, 0.01137831, 0.01104497, 0.01003902)


# Do the HD scenario now
cens_scales_low <- cens_scales_low[p_vals <= 100]
p_vals <- p_vals[p_vals <= 100]


cat("\nStarting with low censoring scenario\n\n")
results_low <- run_simulations_over_p(
  p_values = p_vals,
  cens_scales = cens_scales_low,
  M = M,
  n_train = n_train,
  sigma = sigma,
  s_prog = s_prog,
...
combined_results <- list(results_low = results_low)


# Define output file path 
# (! NAME MUST BE FILENAME_output.rds !)
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_deeper_friedman2_low_LD_output.rds")
